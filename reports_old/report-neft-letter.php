...
//while($row=mysql_fetch_array($resbnk)){
   ...
   $sql11= "SELECT te.netsalary,mc.bankacno as bankacno,mc.comp_name,mb.ifsc_code,te.bankacno as beneficiary_acno ,'10' as type,concat(e.first_name,' ',e.middle_name,' ',e.last_name) AS name, e.pin_code,'".$desc ."' as descri, '".$comp."' as Originator from $tab_emp te inner join employee e on te.emp_id = e.emp_id inner join mast_bank mb on mb.mast_bank_id = te.bank_id inner join mast_company mc on mc.comp_id = te.comp_id where te.comp_id ='".$comp_id."' AND te.user_id='".$user_id."' AND  te.client_id = '".$clintid."' and te.sal_month = '$frdt' and te.netsalary > 0 and te.pay_mode = 'N' ORDER BY te.bank_id, te.Client_id,te.bankacno,e.last_name,e.first_name,e.middle_name,e.Joindate ASC";
  
    $res11 = mysql_query($sql11);
     $sql21 = "select sum(te.netsalary) as netsalary from $tab_emp te  where comp_id ='".$comp_id."' AND user_id='".$user_id."'  and client_id = '".$clintid."'   and te.sal_month = '$frdt'  and te.pay_mode = 'N' ";
  ...
  $stringmoney=makewords($money);
   ...
   if($ecount!='') {
       ...
       <div > 
		 ...
		 <!--<td class='thheading'width = "5%">Sr.No</th> -->
                               <th >Net Salary</th>
                               <th >Sender's AcNo</th>
                               <th >IFSC Code</th>
                           ...
                           ?>
						   <tr> <td><?php echo number_format($money,2,'.',','); ?> </td></tr>
                       </table>
                   </div>
