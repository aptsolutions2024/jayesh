...
if ($pay_type=="L"){
 ...
 $frdt=date("Y-m-d", strtotime($_REQUEST['from_date']));
	   $sql="select  te.*,e.first_name,e.middle_name,e.last_name, cd.check_no,cd.date from leave_details te inner join employee e on te.emp_id = e.emp_id inner join cheque_details cd on cd.emp_id = te.emp_id and cd.payment_date =  '$frdt' and cd.type = 'L'
	 where te.client_id = '$clintid'  and te.pay_mode = 'C' and cd.amount>0  order by e.pay_mode ,te.emp_id";
	
	$resgtemp11 = mysql_query($sql);
	$bankrec = mysql_num_rows($resgtemp11);
	  $reporttitle="CHEQUE LIST FOR THE LEAVE PAYMENT ON  ". date('F Y',strtotime($frdt));
	$_SESSION['reporttitle']=strtoupper($reporttitle);
}
