...
//echo "-".$fdate."-";
 ?>
 <form method="post" action="/add_leave_process" id="leaveupdate">
		 ...
		 foreach($empdate as $emp){
			 ...
			 $getcalculated = $leave->getCalculated($calculationfrm,$calculationto,$emp['emp_id']);
			  ...
			  if($presentday !=0){
			   ...
			   echo "<br>".$encashed = $leave->getEncashment($carfrfrm,$carfrto,$leave_type,$emp['emp_id']);
			   if ($encashed =='')
			   {	echo $obcf =$leave->getleaveCarriedFor($carfrfrm,$carfrto,$emp['emp_id'],$leave_type);
					echo "<br>".$enjoyed = $leave->getEnjoyed($carfrfrm,$carfrto,$emp['emp_id'],$leave_type);
