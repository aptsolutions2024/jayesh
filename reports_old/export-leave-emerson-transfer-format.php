...
//$sql11 = "select te.emp_id,te.netsalary,e.first_name,e.middle_name,e.last_name,te.bankacno from $tab_emp te inner join employee e on te.emp_id = e.emp_id  inner join mast_bank mb on te.bank_id = mb.mast_bank_id   where (mb.bank_name like '%".$bank[$i]."%' ) and  te.comp_id ='".$comp_id."' AND te.user_id='".$user_id."'   and te.client_id  in ( 12,13,14,15,16) and te.sal_month = '$frdt' and te.netsalary > 0 and te.pay_mode = 'T' ORDER BY te.pay_mode,te.Client_id ,te.bankacno,e.last_name,e.first_name,e.middle_name,e.Joindate ASC";
	 ...
	 where  te.client_id  in ( 12,13,14,15,16) and (mb.bank_name like '%IDBI%' or  mb.bank_name like '%KARAD%' ) and te.from_date >= '$frdt1' and to_date <='$to_date'  and e.pay_mode = 'T' and te.amount >0 and te.comp_id ='".$comp_id."'";
	$setRec = mysql_query($sql);

	   $sql = "insert into $tab (  `bankacno` ,`curr_code` , `outlet` , `tran_type` ,`tran_amt` ,`perticulars`  ,`ref_amt` ,`r_cur_code`) select concat(".chr(34).chr(39).chr(34).",mc.`bankacno`),'INR','459','D',sum(te.amount),'$desc',sum(te.amount),'INR' from $tab_emp te  inner join employee e on e.emp_id = te.emp_id   inner join mast_bank mb on te.bank_id = mb.mast_bank_id  inner join mast_company mc on te.comp_id = mc.comp_id   where  te.comp_id  ='".$comp_id ."' and te.from_date >= '$frdt1' and to_date <='$to_date' and te.client_id in ( 12,13,14,15,16)  and mb.bank_name like '%IDBI%' and e.pay_mode = 'T' and te.amount >0 and  te.comp_id ='".$comp_id."' ";;
	$setRec = mysql_query($sql);
