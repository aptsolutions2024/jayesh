...
$setExcelName ="Sumamry_emerson_bill";
...
echo "
<div align='center'><h2>Summary Report For ".date("M Y ", strtotime($frdt))."</h2></div>
...
?>
	<tr>
		...
		<th align= 'right'><?php echo number_format($row2['basic_daily_annex'],2,".",",");?></th>
		<th align= 'right'><?php echo number_format($row2['da_daily_annex'],2,".",",");?></th>
		<th align= 'right'><?php echo number_format($row2['hra_daily_annex'],2,".",",");?></th>
		<th align= 'right'><?php echo number_format($row2['supplli_allow_daily_annex'],2,".",",");?></th>
		...
		<th align= 'right'><?php echo number_format($row2['tot_amount_annex'],2,".",",");?></th>
	</tr>	
	...
	<?php }}
	 ...
	 if ($prev_dept_id=="" || $prev_dept_id != $row['dept_id']){?>
	<tr>
	<td align='right'><?php echo $srno;$srno++;?></td>
	<td align= 'right'><?php echo substr($row['dept_name'],0,4);?></td>
	<td align= 'right'><?php echo $row['dept_name'];?></td>
	 <?php }
	...
	<td align= 'right'><?php echo number_format($row['basic_daily_annex'],2,".",",");?></td>
	<td align= 'right'><?php echo number_format($row['da_daily_annex'],2,".",",");?></td>
	<td align= 'right'><?php echo number_format($row['hra_daily_annex'],2,".",",");?></td>
	<td align= 'right'><?php echo number_format($row['supplli_allow_daily_annex'],2,".",",");?></td>
	...
	<td align= 'right'><?php echo number_format($row['gross']+$row['pf_annex']+$row['other_charges']+$row['supervisioncharges']+$row['servicecharges']+$row['cgst_annex']+$row['sgst_annex'] ,2,".",",");?></td>
</tr>
<?php } 
...
if($tab_client=="1214"){
  $sql = "SELECT  `dept_name`, client_name, count(emp_id) as headcount,sum( `payabledays_sal`)payabledays_sal,sum( `othours_sal`) as othours_sal,sum(  `overtime_sal`) as overtime_sal,sum(`basic_daily_annex`) as basic_daily_annex, sum(`da_daily_annex`) as da_daily_annex, sum(`hra_daily_annex`) as hra_daily_annex, sum(`super_skill_allow_daily_annex`) as super_skill_allow_daily_annex , sum(supplli_allow_daily_annex) as supplli_allow_daily_annex, sum(`other_allow_daily_annex`) as other_allow_daily_annex, sum(total_a_annex) as total_a_annex,sum(total_a_annex+overtime_sal) as gross, sum(`pf_annex`) as pf_annex,sum( `bonus_Annex`+ `esi_annex`+ `lww_annex`+ `lwf_annex` +`safety_annex`+ `soap_annex`+ `training_annex`+tds_annex) as other_charges,sum(supervisioncharges) as supervisioncharges,sum(servicecharges) as servicecharges,sum(cgst_annex) as cgst_annex,sum(sgst_annex) as sgst_annex,sum(cgst_annex+sgst_annex) as gst_annex, sum(`tot_amount_annex`) as tot_amount_annex  FROM $tab where emp_name !='STD_Annex'  and sal_month = '$frdt' and dept_id = ".$prev_dept_id;
	...
	<tr>
		...
		<th align= 'right'><?php echo number_format($row2['overtime_sal'],2,".",",");?></th>
		<th align= 'right'><?php echo number_format($row2['gross'],2,".",",");?></th>
	<td align= 'right'><?php echo number_format($row2['pf_annex'],2,".",",");?></td>
	<td align= 'right'><?php echo number_format($row2['other_charges'],2,".",",");?></td>
	<td align= 'right'><?php echo number_format($row2['gross']+$row2['pf_annex']+$row2['other_charges'],2,".",",");?></td>
	<td align= 'right'><?php echo number_format($row2['supervisioncharges'],2,".",",");?></td>
	<td align= 'right'><?php echo number_format($row2['servicecharges'],2,".",",");?></td>
		
		<th align= 'right'><?php echo number_format($row2['cgst_annex'],2,".",",");?></th>
		<th align= 'right'><?php echo number_format($row2['sgst_annex'],2,".",",");?></th>
		<th align= 'right'><?php echo number_format($row2['gross']+$row2['pf_annex']+$row2['other_charges']+$row2['supervisioncharges']+$row2['servicecharges']+$row2['cgst_annex']+$row2['sgst_annex'] ,2,".",",");?></th>
