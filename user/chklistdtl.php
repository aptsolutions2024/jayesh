...
$userObj=new user(); $month=$_SESSION['month'];
...
$type = "S";
 ...
 if($month=='current'){
  ...
  $monthtit =  date('F Y',strtotime($_SESSION['frdt']));    
	...
	$sal_month = $userObj->getLastDay($frdt);
...
$emplist = $userObj->getChequeEmployeeByClientId($clientid,$tab_emp,$sal_month);
 ...
 <form method="post"  method="post" action="/add_cheque_details" id="addchkdtl" >
	...
	while($emp = $emplist->fetch_assoc()){
		$chkDtl = $userObj->chkDetails($emp['emp_id'],$sal_month,$type);
		//print_r($chkDtl);
		$cnt = 0;
		$cnt = mysqli_num_rows($chkDtl);	
		$chk = $chkDtl->fetch_assoc();;
	...
	<?php $srno++; $no++;} ?>
			...
			<div>			
<div><input type="submit" value="Save" name="submit" class="btnclass" id="addcheque" > <input type="submit" value="Print All" name="printbtn" class="btnclass"  onclick="printval()">	 <input type="submit" value="Export" name="exportbtn" class="btnclass" onclick = "export1();" >
</div>
            </form>
        </div>
		<script>
	...
	if(parseInt(print1) == 1){	
		...
		var empid = $("#empid").val();		
		window.location.href="check-print-pre.php?client="+client+"&empid1="+empid+"&checkdate="+chkdt+"&empid="+empid+"&type=S";
