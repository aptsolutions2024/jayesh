...
$row= mysql_query($sql);
 ...
 if($emp=='Parent')
 {
	 ...
	 where  t3.parentid='".$clintid."' and te.sal_month = '".$frdt."' and e.pay_mode = 'T'";
   $setRec = mysql_query($sql);

	 $sql = "insert into $tab (  `bankacno` ,`curr_code` , `outlet` , `tran_type` ,`tran_amt` ,`perticulars`  ,`ref_amt` ,`r_cur_code`) select concat(".chr(34).chr(39).chr(34).",mc.`bankacno`),'INR','459','D',sum(te.netsalary),'$desc',sum(te.netsalary),'INR' from $tab_emp te inner join employee e on e.emp_id = te.emp_id  inner join mast_company mc on te.comp_id = mc.comp_id inner join mast_client t3 on te.client_id = t3.mast_client_id  where  te.comp_id  ='".$comp_id ."' and te.sal_month = '".$frdt."' and te.parentid ='".$clintid."'  and e.pay_mode = 'T'";
	$setRec = mysql_query($sql);
	 ...
	 $sql = "insert into $tab (  `bankacno` ,`curr_code` , `outlet` , `tran_type` ,`tran_amt` ,`perticulars`  ,`ref_amt` ,`r_cur_code`) select concat(".chr(34).chr(39).chr(34).",te.`bankacno`),'INR','459','C',te.netsalary,'$desc',te.netsalary,'INR' from $tab_emp te  inner join employee e on e.emp_id = te.emp_id  inner join mast_client t3 on te.client_id= t3.mast_client_id
	 where  te.client_id ='".$clintid."' and te.sal_month = '".$frdt."'  and e.pay_mode = 'T'";
	$setRec = mysql_query($sql);

	 $sql = "insert into $tab (  `bankacno` ,`curr_code` , `outlet` , `tran_type` ,`tran_amt` ,`perticulars`  ,`ref_amt` ,`r_cur_code`) select concat(".chr(34).chr(39).chr(34).",mc.`bankacno`),'INR','459','D',sum(te.netsalary),'$desc',sum(te.netsalary),'INR' from $tab_emp te  inner join employee e on e.emp_id = te.emp_id inner join mast_company mc on te.comp_id = mc.comp_id   where  te.comp_id  ='".$comp_id ."' and te.sal_month = '".$frdt."' and te.client_id ='".$clintid."'  and e.pay_mode = 'T'";
	$setRec = mysql_query($sql);
