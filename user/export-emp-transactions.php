...
//$setSql = "select 'client_id','client_name','Sal_month','Emp_Id' ,' Employee_Name', 'Present','Absent','Weekly  off','Pl','Sl','El','Paid Holiday','Additional Days','Extra Income 1','Extra Income 2','Extra Deduct 1','Extra Deduct 2','Left Date','Invalid' union all SELECT mc.mast_client_id ,mc.client_name,mc.current_month as 'Sal_month',emp.`emp_id` as 'Emp_ID',concat(emp.first_name,' ',emp.last_name) AS 'Employee_NAME', '', '', '', '', '', '', '', '', '', '', '', '', '', '' FROM `employee` emp inner join mast_client mc  on emp.client_id = mc.mast_client_id where emp.client_id = '".$client_id."' and emp.job_status != 'L'  order by employee_name";	
  ...
  $j= count($exhd);
$setRec = $userObj->exportEmpTransaction($exhd,$client_id,$j,$comp_id,$user_id);
...
for($i=0; $i<$j; $i++ ){
  ...
  $setSql .= ",emp.ticket_no,md.mast_dept_name as 'CC_Code' FROM `employee` emp inner join mast_client mc  on emp.client_id = mc.mast_client_id  inner join mast_dept md on emp.dept_id= md.mast_Dept_id where emp.client_id = '".$client_id."' and emp.job_status != 'L'  order by EMP.EMP_ID";	
*/
//.="'' as Present , '' as Absent,' ' as Weeklyoff,' ' as Pl,' ' as Sl,' ' as El,' ' as Paid_Holiday,' ' as Additional_Days,' ' as Extra_Income_1,' ' as Extra_Income_2,' ' as Extra_Deduct_1,' ' as Extra_Deduct_2 FROM `employee` emp inner join mast_client mc  on emp.client_id = mc.mast_client_id where emp.client_id = '".$client_id."' and emp.job_status != 'L'  order by EMP.EMP_ID";	
