...
$rowchk = mysql_query($sqlchk);
 ...
 if ($comptype=="new"){
 $sql2 = "select distinct(e.emp_id),e.client_id,e.bank_id,e.bankacno,e.pay_mode from employee e inner join hist_employee he on e.emp_id=he.emp_id where e.client_id='".$client."' and he.sal_month>= '".$startday."' and he.sal_month <= '".$endday."' ";
 }
 else
 {$sql2 = "select distinct(e.emp_id),e.client_id, e.bank_id,e.bankacno,e.pay_mode from employee e inner join hist_employee he on e.emp_id=he.emp_id where he.client_id='".$client."' and he.sal_month>= '".$startday."' and he.sal_month <= '".$endday."' ";
 }
...
while($res2 = mysql_fetch_array($row2)){
	...
	if($empnum !=0){
		 echo " Record has een deleted for  Employee  : ".$res2['emp_id'] ." from Client : "+$res2['client_id']." <br>";
		$sqlchk ="delete from bonus where emp_id='".$res2['emp_id']."' and from_date = '".$startday."' and todate ='".$endday."'";
		$rowchk = mysql_query($sqlchk);
		...
		echo $lastinsid = mysql_insert_id();
	 ...
	 $sql3 ="select sum(ti.amount) amount,te.emp_id empid, mih.income_heads_name head_name,
		...
		while($row3 = mysql_fetch_array($res3)){
			...
			if($type =='2' &&  $row3['amount'] != $amount){
				echo $row3['amount']."      "    .$amount."<br>";
				$amt = $amount;	
				
				echo $sqldate = "SELECT day(LAST_DAY('".date('y-m-d',strtotime($row3['sal_month']))."')) AS monthdays";
				$rowdate= mysql_query($sqldate);
				$resdate = mysql_fetch_assoc($rowdate);
				$monthdays = $resdate['monthdays'];
				echo $monthdays."    ".$amount."<br>";
			...
			$exgratiaamt = $amt*$exgratia/100; //default 11.67
			 ...
			 $update1 = "update bonus set ".$monthwages."_wages = '".$row3['amount']."',".$monthwages."_bonus_wages = '".$amt."',".$monthwages."_payable_days = '".$row3['payabledays']."',".$monthwages."_bonus_amt = '".$bonusamt."',".$monthwages."_exgratia_amt = '".$exgratiaamt."' where emp_id ='".$res2['emp_id']."' and  from_date = '".$startday ."' and todate = '".$endday."'";			
			mysql_query($update1);
		...
		echo	 $sql3 ="select sum(ti.amount) amount,te.emp_id empid, mih.income_heads_name head_name,
		...
		while($row3 = mysql_fetch_array($res3)){
			...
			if($type =='2' &&  $row3['amount'] != $amount){
				...
				$sqldate = "SELECT day(LAST_DAY('".date('y-m-d',strtotime($row3['sal_month']))."')) AS monthdays";
				$rowdate= mysql_query($sqldate);
				$resdate = mysql_fetch_assoc($rowdate);
				$monthdays = $resdate['monthdays'];
				echo $monthdays."    ".$amount."<br>";
			...
			$selbon ="select emp_id,round(apr_bonus_amt + may_bonus_amt + jun_bonus_amt + jul_bonus_amt + aug_bonus_amt + sep_bonus_amt + oct_bonus_amt + nov_bonus_amt + dec_bonus_amt + jan_bonus_amt + feb_bonus_amt + mar_bonus_amt+0.01,0) as bonustotal,
