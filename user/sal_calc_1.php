...
// step 1. checking for left employees
	...
	echo $sql = "SELECT trd_id FROM tran_days td inner join employee  emp  on emp.emp_id=td.emp_id where td.present+td.weeklyoff+td.paidholiday+td.sl+td.cl+td.pl+td.absent+td.otherleave != ('".$monthdays ."'-day(emp.joindate))+1 and td.leftdate ='0000-00-00' and  td.client_id = '".$client_id."'and  emp.joindate> '".$startmth."'"; 
	$row= mysql_query($sql);
	print_r ($row);
	if(       mysql_num_rows($row) !=0)
	...
	echo $sql = "SELECT trd_id FROM tran_days td inner join employee  emp  on emp.emp_id=td.emp_id where td.present+td.weeklyoff+td.paidholiday+td.sl+td.cl+td.pl+td.absent+td.otherleave != ( day(td.leftdate) - day('".$startmth."'))+1 and td.leftdate !='0000-00-00' and  td.client_id = '".$client_id."' and  emp.joindate< '".$startmth."'" ;
	$row= mysql_query($sql);
	if(       mysql_num_rows($row) !=0)
		{echo "Invalid Total Days.Please Check Transaction Days Details.";
			while($res = mysql_fetch_assoc($row)){
...
echo $sqltab = "update tran_days td inner join emp_leave el on el.emp_id = td.emp_id  set td.plbal = el.ob-el.enjoyed  where el.leave_type_id = 5";
$rowtab= mysql_query($sqltab);
...
//$sql = "update tab_inc".$user_id."  ti  inner join tab_emp".$user_id ." te on te.emp_id = ti.emp_id inner join tran_days td on td.emp_id = ti.emp_id  set ti.amount = ti.std_amt*te.payabledays  where ti.calc_type= 5 and te.client_id = '".$client_id ."'";
//$row= mysql_query($sql);
...
while($row2 = mysql_fetch_assoc($row1))
{
	$sql21 = "insert into tab_inc".$user_id ." (emp_id,head_id,std_amt,amount,sal_month,calc_type) values ('".$row2['emp_id']."' ,'".$row31['head_id']."','0','".$row2['extra_ded1']."','".$cmonth."','0')";
	$row22= mysql_query($sql21);	
...
while($row2 = mysql_fetch_assoc($row1))
{
	$sql21 = "insert into tab_inc".$user_id ." (emp_id,head_id,std_amt,amount,sal_month,calc_type) values ('".$row2['emp_id']."' ,'".$row31['head_id']."','0','".$row2['extra_ded2']."','".$cmonth."','0')";
	$row22= mysql_query($sql21);	
...
$sql = "update tab_emp".$user_id ." te  inner join tran_days td on te.emp_id = td.emp_id set te.netsalary = te.gross_salary - te.tot_deduct+td.extra_inc2 where  te.client_id ='".$client_id ."'";
$row= mysql_query($sql);
