...
$X = $stryr[0]-1;
$Y = $stryr[1]-1;
 $bonus_fromdate=$X."-04-30";
 $bonus_todate="20".$Y."-03-31";
...
//history and tran conveyance 
 ...
 $sql="insert into tab_itcalc".$user_id ."( `emp_id`, `conveyance`,`proj_months`,sal_month,`year`) select emp_id,amount,'$mfactor',sal_month,'$year' from hist_income where sal_month>='$fromdate' AND sal_month<='$todate' AND head_id in(select mast_income_heads_id from mast_income_heads  where  ((`income_heads_name` LIKE '%Conveyance%' OR `income_heads_name` LIKE '%TRAVEL%')  AND comp_id='$comp_id'))  " .$Empidval;
$rowt= mysql_query($sql);

 $sql="insert into tab_itcalc".$user_id ."( `emp_id`, `conveyance`,`proj_months`,sal_month,`year`) select emp_id,amount,'$mfactor',sal_month,'$year' from tran_income where sal_month>='$fromdate' AND sal_month<='$todate' AND head_id in(select mast_income_heads_id from mast_income_heads  where  ((`income_heads_name` LIKE '%Conveyance%' OR `income_heads_name` LIKE '%TRAVEL%')  AND comp_id='$comp_id'))  " .$Empidval;
$rowt= mysql_query($sql);
...
echo $sql="insert into tab_itcalc".$user_id ."( `emp_id`, `esi`,`proj_months`,sal_month,`year`) select emp_id,amount,'$mfactor',sal_month,'$year' from hist_deduct where sal_month>='$fromdate' AND sal_month<='$todate' AND head_id in(select mast_deduct_heads_id from mast_deduct_heads  where  `deduct_heads_name` LIKE '%E.S.I.%'  and comp_id = '".$comp_id."')".$Empidval;
$rowt= mysql_query($sql);
echo "<br>";
...
if($salmounth>=$fromdate && $salmounth<=$todate){
   ...
   $sql="update tab_itcalc1".$user_id ." ti inner join employee e on  e.emp_id=ti.emp_id set proj_months=(12-$salm+3)) where e.joindate<='$fromdate' AND (e.leftdate>'$todate' OR e.leftdate!='0000-00-00') ".$Empidval;
      ...
      $sql="update tab_itcalc1".$user_id ." ti inner join employee e on  e.emp_id=ti.emp_id set proj_months=((12-Ssalm+3)) where e.joindate>'$fromdate' AND (e.leftdate>'$todate' OR e.leftdate!='0000-00-00') ".$Empidval;
        ...
        echo $sql="update tab_itcalc1".$user_id ." set gross_salary=gross_salary+(gross_salary*proj_months/($salm-month(e.joindate))),conveyance=conveyance+(conveyance*proj_months/(12-proj_months)),hra=hra+(hra*proj_months/(12-proj_months)),prof_tax=prof_tax+(prof_tax*proj_months/(12-proj_months)),pf=pf+(pf*proj_months/(12-proj_months)),esi=esi+(esi*proj_months/(12-proj_months))".$Empidval;
        $rowt= mysql_query($sql);
...
$rowt= mysql_query($sql);
  ...
  $sql="update tab_itcalc1".$user_id ." set gross_salary=gross_salary+(gross_salary*proj_months/sal_count),conveyance=conveyance+(conveyance*proj_months/sal_count),hra=hra+(hra*proj_months/sal_count),prof_tax=prof_tax+(prof_tax*proj_months/sal_count),pf=pf+(pf*proj_months/sal_count),esi=esi+(esi*proj_months/sal_count)".$Empidval1;
$rowt= mysql_query($sql);

//Adding bonus and leave payment details 
$sql="update tab_itcalc1".$user_id ." ti inner join leave_details ld  on  ld.emp_id=ti.emp_id set  ti.gross_salary=ti.gross_salary +ld.amount  Where  ld.payment_date >= '$fromdate' AND ld.payment_date<='$todate'";
$rowb= mysql_query($sql);

 $sql="update tab_itcalc1".$user_id ." ti inner join bonus b  on  b.emp_id=ti.emp_id set  ti.gross_salary=ti.gross_salary +b.tot_bonus_amt+b.tot_exgratia_amt  Where  b.from_date = '$bonus_fromdate' AND b.todate='$bonus_todate'";
$rowb= mysql_query($sql);
...
{$mheaex1 = 0;
 $mhraex2 = 0;
 $mhraex3 = 0; 
   echo  $mhraex1 = round($row11['hra'],0);
   echo "<br>";
     $sql2= "select hrapaid from it_file2 where year = '".$year."' and emp_id = '".$row11['emp_id']."' and hrapaid > 0";
	$row2= mysql_query($sql2);
	$row21= mysql_fetch_array($row2);
    echo  "HRAPAID : " . $row21['hrapaid'];
	//-($row11['gross_salary']*.1);
	
	echo "<br>";
	echo "gross_salary ". $row11['gross_salary']."   .5% - ";
	echo $mhraex2 = $row11['gross_salary']*.5;
	echo "<br>";

		
     $sql1= "SELECT sum(std_amt) as amount FROM emp_income  WHERE   emp_id = '".$row11["emp_id"]."' and head_id in (select mast_income_heads_id from mast_income_heads  where  (`income_heads_name` LIKE '%BASIC%' or `income_heads_name` LIKE '%D.A.%' OR `income_heads_name` LIKE '%wage%' ) and comp_id = '".$comp_id."'  )";
	
	$row2 = mysql_query($sql1);
	$row3 = mysql_fetch_array($row2);
	echo "<br>";
	echo "BASIC_DA  ".$row3['amount']."  ";
	echo $mhraex3 = $row21['hrapaid']-($row3['amount']*.1*12);
	
	
		
	//echo $mhraex3 = $row11['hra'];
		echo "<br>";
		echo $mhraex1." " .$mhraex2. "  ".$mhraex3; 
...
$rowt= mysql_query($sql);
 ...
 $sql="select * from tab_itcalc1".$user_id ." where year='".$year."' ".$Empidval;
 $row= mysql_query($sql);
...
while($res= mysql_fetch_array($row)){
    ...
    $sql2="INSERT INTO tab_it1_".$user_id ."(`year`,`from_date`,`to_date`, `emp_id`, `col_1`,`col_2_1`, `col_4b_1`,`col_9a_1`,comp_id,user_id) VALUES ('$year','$fromdate','$todate','$tempempid','$tempgross_salary','$tempconveyancehra','$tempprof_tax','$temppfesi','$comp_id','$user_id')";
    $row2= mysql_query($sql2);
...
$sql4=="update tab_it1_".$user_id ." set col_9b_3 = '".$limit_80c." where  col_9b_3 > ".$limit_80c."  where ti.year='".$year."' ".$inEmpidval;
$row4= mysql_query($sql4);
...
$sql4="update tab_it1_".$user_id ." set col_2_2=col_2_1,col_1d_2=col_1+col_1a_1+col_1b_1+col_1c_1,col_3_1=col_1d_2-col_2_2, col_4a_1=0,col_5_1=col_4a_1+col_4b_1,col_6_2=col_3_1-col_5_1,col_8_2=col_6_2+col_7_2,col_9a_2=col_9a_1,col_9c_3 = col_9a_2+col_9b_2+col_9b_3 where year='".$year."' ".$Empidval;
$row4= mysql_query($sql4);
...
while($res=mysql_fetch_array($row41)){
    ...
    $sql44 = "update tab_it1_".$user_id ." set col_12_2=".$tax_val." where  year = '".$res['year'] ."' and emp_id = '". $res['emp_id']."' ";
    $row44 = mysql_query($sql44);

}


 $sql4="update tab_it1_".$user_id  ."  set col_13_2=col_12_2*0.03 where year='".$year."'".$Empidval;
$row4= mysql_query($sql4);

$sql4="update tab_it1_".$user_id ."  set col_14_2=col_12_2+col_13_2 where year='".$year."'".$Empidval;
$row4= mysql_query($sql4);
