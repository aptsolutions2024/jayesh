...
//$bank = $common->getBankDetailsByName('IDBI','KARAD');
//echo $sql = "select * from mast_bank where bank_name like '%IDBI%' or bank_name like '%KARAD%'";
...
<?php 
  $sql11 = "select te.emp_id,te.tot_bonus_amt,e.first_name,e.middle_name,e.last_name,te.bankacno from $tab_emp te inner join employee e on te.emp_id = e.emp_id  inner join mast_bank mb on te.bank_id = mb.mast_bank_id   where (mb.bank_name like '%".$bank[$i]."%' ) and  te.comp_id ='".$comp_id."' AND te.user_id='".$user_id."'   and te.client_id  in ( 12,13,14,15,16) and te.from_date >= '$startbon' and te.todate <= '$endbon' and te.tot_bonus_amt > 0 and te.pay_mode = 'T' ORDER BY te.pay_mode,te.Client_id ,te.bankacno,e.last_name,e.first_name,e.middle_name,e.Joindate ASC";
  
    $res11 = mysql_query($sql11);
    $sql21 = "select sum(te.tot_bonus_amt) as tot_bonus_amt from $tab_emp te inner join mast_bank mb on te.bank_id = mb.mast_bank_id   where  (mb.bank_name like '%".$bank[$i]."%' )  and te.comp_id ='".$comp_id."' AND te.user_id='".$user_id."'  and te.client_id  in ( 12,13,14,15,16)  and te.from_date >= '$startbon' and te.todate <= '$endbon'  and te.pay_mode = 'T'";
 
    $res21 = mysql_query($sql21);
    $row21=mysql_fetch_array($res21);
   $money=round($row21['tot_bonus_amt']);
   //$money = 23345;
           ...
           <div class="row">
               <div>
                   ...
                   <div>
                       ...
                       for crediting Bonus to the individual account of the persons shown
                       ...
                       <table width = "90%">
                           ...
                           while ($rowemp = mysql_fetch_array($res11)) {                              
                               ...
                               <tr>
                                   ...
                                   <td><?php echo $salarynet = $rowemp['tot_bonus_amt']; //$allsalarynet += $salarynet;?></td>
                               </tr>
...
$comapnydtl = $userObj->showCompdetailsById($comp_id);
 echo $comapnydtl['comp_name'];?><br>
	<?php if($comapnydtl['address'] !=""){ ?><br><?php echo $comapnydtl['address']; } if($comapnydtl['addr_1'] !=""){?><br><?php echo $comapnydtl['addr_1']; } if($comapnydtl['addr_2']!=""){?><br><?php echo $comapnydtl['addr_2']; }?>,<br>Pune 411 016<br>Tel. :<?php echo $comapnydtl['tel']; ?> Fax:25656144<br>e-mail:<?php echo $comapnydtl['email']; ?>
	</div>
	</td></tr></thead> 
</table>
<!--</div>-->
